...
| Ptr64/Address64 定义引用 rbf-interface.md §2.2 | 术语表 | ✅ 正确 |
...
| **P3** | 新版术语表 Ptr64/Address64 引用 rbf-interface.md，但未说明两者关系 | 建议补充："`Address64` 是 `Ptr64` 的语义子类型（指向 Frame 起点）；详见 rbf-format.md §9.3" |
...
| **P1** | L1-ALIGN-TESTVECTORS-RBF-IDS | `mvp-test-vectors.md` 中 RBF 相关向量仍绑定 `[F-*]/[R-*]`（原 Layer 1）而非 `rbf-format.md` 的 `[E-*]`（现 Layer 0 SSOT） | 条款↔测试映射断裂，未来 CI/审计无法机械对齐 | 将 RBF framing/resync/CRC/ptr/datatail 的向量断言改为引用 `[E-*]`；StateJournal 自身语义（DiffPayload/_dirtyKeys/commit 不变式）继续引用 `[S-*]/[A-*]/[F-*]`（Layer 1） |
...
| **P2** | L1-ALIGN-PTR64-ADDRESS64 | `mvp-design-v2.md` Glossary 中 `Ptr64/Address64` 合并叙述，容易再次引入“DataTail 也叫 Address64？”的歧义 | pointer 校验点（ptr 指向 HeadLen vs ptr 指向 EOF）会写错，难以测试 | 在 `mvp-design-v2.md` Glossary 补充：`Address64` 是“指向 Frame 起点”的语义子类型；`DataTail` 是 Ptr64 但不是 Address64；并引用 `rbf-format.md §9.3` 与 `rbf-interface.md §2.2` |
