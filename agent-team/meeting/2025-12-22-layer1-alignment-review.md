...
重构后的 `mvp-design-v2.md` 现在仅依赖接口文档 `elog-interface.md`。

需要复核：`elog-interface.md + mvp-design-v2.md` 是否完整覆盖原版 `mvp-design-v2.md.bak` 中的 **Layer 1 内容**。

## 审阅目标

1. **完整性**：原版中的 Layer 1 条款是否都保留在 mvp-design-v2.md？
2. **依赖正确性**：mvp-design-v2.md 对 ELOG 层的引用是否正确？
3. **无冗余**：已提取到 elog-format.md 的内容是否从 mvp-design-v2.md 移除？

## 目标文件

- 原版：`atelia/docs/StateJournal/mvp-design-v2.md.bak`
- 新版：`atelia/docs/StateJournal/mvp-design-v2.md`
- 接口：`atelia/docs/StateJournal/elog-interface.md`
...
5. 对 ELOG 层的引用是否正确（应引用 elog-interface.md，而非内联重复）
...
| ~~`[F-MAGIC-IS-FENCE]`~~ | ⚠️ 已移除 | **正确移除**：属于 Layer 0，现在 elog-format.md |
| ~~`[F-MAGIC-RECORD-SEPARATOR]`~~ | ⚠️ 已移除 | **正确移除**：属于 Layer 0，现在 elog-format.md |
| ~~`[F-HEADLEN-TAILLEN-SYMMETRY]`~~ | ⚠️ 已移除 | **正确移除**：属于 Layer 0，现在 elog-format.md |
| ~~`[F-RECORD-4B-ALIGNMENT]`~~ | ⚠️ 已移除 | **正确移除**：属于 Layer 0，现在 elog-format.md |
| ~~`[F-CRC32C-PAYLOAD-COVERAGE]`~~ | ⚠️ 已移除 | **正确移除**：属于 Layer 0，现在 elog-format.md |
| ~~`[F-RECORD-WRITE-SEQUENCE]`~~ | ⚠️ 已移除 | **正确移除**：属于 Layer 0，现在 elog-format.md |
...
| ~~`[R-RESYNC-DISTRUST-TAILLEN]`~~ | ⚠️ 已移除 | **正确移除**：属于 Layer 0，现在 elog-format.md |
...
| 新版引用 | 位置 | 状态 |
|----------|------|------|
| "依赖文档：elog-interface.md" | 文档顶部 | ✅ 正确 |
| "帧格式详见 elog-format.md" | §3.2.1 | ✅ 正确 |
| "详见 elog-format.md" | §3.5 | ✅ 正确 |
| "编码基础：本节使用的变长整数编码（varint）定义见 elog-format.md" | §3.2 | ✅ 正确 |
| Ptr64/Address64 定义引用 elog-interface.md §2.2 | 术语表 | ✅ 正确 |

##### 3.2 引用语义检查

新版正确将以下内容标记为"依赖 ELOG 层"而非重复定义：
...
| 级别 | 问题 | 建议 |
|------|------|------|
| **P2** | `[R-META-RESYNC-SAME-AS-DATA]` 条款在原版明确声明 "meta 文件尾部也采用与 data 相同的 resync 策略"，新版未保留此条款引用 | 建议在 §3.2.2 或 §3.5 添加说明："Meta 文件的 Resync 策略与 Data 文件相同，详见 [elog-format.md](elog-format.md) `[E-RESYNC-SCAN-MAGIC]`" |
| **P3** | 新版术语表 Ptr64/Address64 引用 elog-interface.md，但未说明两者关系 | 建议补充："`Address64` 是 `Ptr64` 的语义子类型（指向 Frame 起点）；详见 elog-format.md §9.3" |
| **P3** | 新版 §3.2.2 删除了原版的"补充：meta 文件尾部也可能存在随机垃圾..."说明段落 | 建议恢复此说明，或添加交叉引用到 elog-format.md |
...
| **依赖正确性** | ✅ 正确引用 elog-interface.md 和 elog-format.md |
...
- 我同意 Claude 对“业务语义（对象模型 / Two-phase commit / DiffPayload / 生命周期不变式）整体保留”的判断：新版 [atelia/docs/StateJournal/mvp-design-v2.md](../../atelia/docs/StateJournal/mvp-design-v2.md) 的 Layer 1 主体内容确实比原版更聚焦，且对 ELOG 的引用方向基本正确。
- 但在“精确性/可测试性”维度上，当前存在 2 个会直接导致实现分叉/测试无法落地的 **对齐断点**：
	1) **RecordKind ↔ FrameTag 的判别器语义不一致**（`mvp-design-v2.md` vs `elog-interface.md`/`elog-format.md`）。
	2) **varint 的精确定义在 Layer 1 文档中不再是可寻址 SSOT**（原版有 §3.2.0.1；新版仅引用，但被引用目标 `elog-format.md` 并未提供 varint 规范）。
...
- Claude 的表格里将 `[F-VARINT-CANONICAL-ENCODING]`、`[F-DECODE-ERROR-FAILFAST]` 标为“新版保留（§3.2.0.1）”。但新版 `mvp-design-v2.md` 已不再包含原版的 “3.2.0.1 varint 精确定义”段落；并且当前被引用的 [atelia/docs/StateJournal/elog-format.md](../../atelia/docs/StateJournal/elog-format.md) 也没有 varint 章节。
	- 结果：**varint 规范在当前 SSOT 链路上是缺失的**（可测试性缺口）。
- Claude 将 `[R-META-RESYNC-SAME-AS-DATA]` 视为“可能需要恢复”。我倾向于：
	- 若 Layer 1 已完全通过 `IElogScanner.ScanReverse()` 获得“已通过 ELOG framing/CRC/resync 过滤后的 frame 流”，那么“meta resync 与 data 相同”应当归属在 `elog-format.md` 的 `[E-RESYNC-*]` 条款中，而不应作为 Layer 1 的 `[R-*]` 再重复。
	- 但前提是：Layer 1 文档必须明确 **Open() 的输入是“ELOG 层已完成 resync 的 frame 枚举”**，而不是“自己做 raw bytes reverse scan”。否则测试会不知道该在哪一层断言 resync 行为。
...
- `elog-format.md`（Layer 0）已经把 **FrameTag 的 wire encoding**钉死：Tag 是 Payload 的第 1 个字节（`[E-FRAMETAG-WIRE-ENCODING]`），Frame 布局为 `HeadLen + Payload(Tag+Body) + Pad + TailLen + CRC32C + Magic`。
- `elog-interface.md`（接口）在 §5.1 规定 StateJournal 的映射：
...
- 若实现者遵循 `elog-interface.md`：会以 `frame.Tag` 做分流（0x01 解析 ObjectVersionPayload；0x02 解析 MetaCommitPayload）。此时 payload 内是否还需要 `RecordKind` 字节？如果 `mvp-design-v2.md` 仍要求它存在，那么 payload 的第一个字节就会被“双重占用”（既是 FrameTag，又是 RecordKind），解析会立刻错位。
- 若实现者遵循 `mvp-design-v2.md`：会在 payload 内读 `RecordKind`（0x01），并按“data/meta 域隔离”来区分同值含义；但这与 `elog-interface.md` 的“统一 FrameTag 值空间”矛盾。

**2.3 建议的收敛方式（从可测试性角度）**

我建议以“唯一判别器”原则收敛（避免双轨）：

- **推荐方案（更贴合现有 elog-format/elog-interface）：FrameTag 是唯一顶层判别器**。
...
- 新版 `mvp-design-v2.md` 仍大量使用 `varuint/varint` 作为字段类型（例如 `EpochSeq`、`ObjectId`、`PairCount`、`KeyDeltaFromPrev` 等），但把“编码基础”改为引用 `elog-format.md`。
- 然而 `elog-format.md` 目前不包含 varint 章节（它是 ELOG framing 文档，内容聚焦 HeadLen/CRC/Magic/Resync/Ptr64/DataTail）。
...
1) **与分层后的条款 ID 体系不一致**：文件仍用 `[F-*]/[R-*]` 绑定大量 ELOG framing/resync/CRC 行为，但这些行为现在 SSOT 已迁移到 `elog-format.md` 的 `[E-*]` 条款。
2) **与判别器语义不一致**：向量文本多处把“payload 第 1 字节 = kind”作为前提，但现在 `elog-format.md` 已把 “payload 第 1 字节 = FrameTag”写死；同时 `elog-interface.md` 又给出了 FrameTag 的业务映射。

另外，文件内部存在“ELOG framing”章节重复（先是 §5，再出现一个“## 1. ELOG framing（data/meta）”），这会造成未来 drift（同一向量/规则被双写）。
...
| 级别 | ID | 问题 | 影响 | 建议修复动作 |
|------|----|------|------|--------------|
| **P0** | L1-ALIGN-RECORD-DISCRIMINATOR | `mvp-design-v2.md` 的“payload 内 RecordKind + 域隔离复用 0x01”与 `elog-interface.md` 的“FrameTag 0x01/0x02 映射”以及 `elog-format.md` 的“Tag 是 Payload 第 1 字节”冲突 | 读写器必然错位解析，测试无法写成黑盒断言（会出现两套实现） | 选择并锁死“唯一判别器”：**推荐以 FrameTag 为唯一顶层判别器**；更新 `mvp-design-v2.md` 的 record payload 定义与 `elog-interface.md §5.1`/`elog-format.md` 完全一致；如需保留 RecordKind 则必须改为 `RecordKind == FrameTag` 并删除“域隔离复用”表述 |
...
| **P1** | L1-ALIGN-TESTVECTORS-ELOG-IDS | `mvp-test-vectors.md` 中 ELOG 相关向量仍绑定 `[F-*]/[R-*]`（原 Layer 1）而非 `elog-format.md` 的 `[E-*]`（现 Layer 0 SSOT） | 条款↔测试映射断裂，未来 CI/审计无法机械对齐 | 将 ELOG framing/resync/CRC/ptr/datatail 的向量断言改为引用 `[E-*]`；StateJournal 自身语义（DiffPayload/_dirtyKeys/commit 不变式）继续引用 `[S-*]/[A-*]/[F-*]`（Layer 1） |
...
| **P2** | L1-ALIGN-PTR64-ADDRESS64 | `mvp-design-v2.md` Glossary 中 `Ptr64/Address64` 合并叙述，容易再次引入“DataTail 也叫 Address64？”的歧义 | pointer 校验点（ptr 指向 HeadLen vs ptr 指向 EOF）会写错，难以测试 | 在 `mvp-design-v2.md` Glossary 补充：`Address64` 是“指向 Frame 起点”的语义子类型；`DataTail` 是 Ptr64 但不是 Address64；并引用 `elog-format.md §9.3` 与 `elog-interface.md §2.2` |
| **P2** | L1-ALIGN-TESTVECTORS-DEDUP | `mvp-test-vectors.md` 出现重复的 “ELOG framing”章节块 | 未来维护必然 drift | 合并为单一章节树，避免同一规则/向量被双写 |

---

#### 6. 我建议的“对齐收敛检查点”（便于 Implementer/QA 落地）

- 文档层面：读者只凭 `elog-interface.md + mvp-design-v2.md` 能否写出 **唯一** 的“从 frame.Tag 决定 payload 解析器”的流程？
...
| elog-format.md | — | 572 | 新建 |
| elog-interface.md | — | 395 | 新建 |
