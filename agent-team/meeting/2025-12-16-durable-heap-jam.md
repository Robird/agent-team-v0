...
graph LR
    subgraph "传统思维"
        RAM1[RAM: Model] -->|定期 Save| Disk1[Disk: Snapshot]
        Disk1 -->|启动时 Load| RAM1
    end
    
    subgraph "DurableHeap 思维"
        Disk2[Disk: Model 本尊] <-->|mmap| RAM2[RAM: Model 投影]
    end
```
...
graph TB
    subgraph "传统进程"
        P1[进程启动] --> P2[分配内存] --> P3[执行代码]
        P3 --> P4[进程结束]
        P4 --> P5[内存消失!]
    end
    
    subgraph "Durable Process (DurableHeap)"
        D1[进程启动] --> D2[mmap 打开 Heap]
        D2 --> D3[从上次的 Root 继续执行]
        D3 --> D4{进程结束?}
        D4 -->|Crash| D5[重启，回到 D2]
        D4 -->|正常退出| D6[Root 保留在 Heap]
        D6 --> D7[下次启动继续]
    end
```
