...
# 版本: 2.0.0
# 创建日期: 2025-12-31
# 更新日期: 2026-01-01 - 添加 Artifact Tiers 和 L1-L5 检查
...
echo -e "${BLUE}=== 术语一致性检查脚本 v2.0.0 ===${NC}"
echo "开始时间: $(date)"
echo ""
...
for dir in "${SPEC_DIRS[@]}"; do
    if [ -d "$dir" ]; then
        for pattern in "${LAYER_PATTERNS[@]}"; do
            ...
            matches=$(grep -r "$pattern" --include="*.md" "$dir" 2>/dev/null | grep -v "❌\|代码标识符域\|过渡期\|历史文档" | wc -l)
...
# 4. 检查 Artifact Tiers 使用情况
echo -e "\n${BLUE}[4/6] 检查 Artifact Tiers 使用情况...${NC}"

# 检查是否使用了正确的 Artifact Tiers 术语
TIERS_CORRECT_PATTERNS=(
    "Artifact Tiers"
    "Artifact Tier"
    "Tiers 框架"
    "Tier:"
...
for dir in "${SPEC_DIRS[@]}"; do
    if [ -d "$dir" ]; then
        # 检查正确术语的使用
        correct_count=0
        for pattern in "${TIERS_CORRECT_PATTERNS[@]}"; do
            count=$(grep -r "$pattern" --include="*.md" "$dir" 2>/dev/null | wc -l)
            correct_count=$((correct_count + count))
        done
        
        if [ "$correct_count" -gt 0 ]; then
            echo -e "  ${GREEN}✅ 在 $dir 中发现 $correct_count 处正确使用 Artifact Tiers 术语${NC}"
        fi
        
        # 检查错误术语的使用（排除历史文档和过渡期说明）
        for pattern in "${TIERS_INCORRECT_PATTERNS[@]}"; do
            matches=$(grep -r "$pattern" --include="*.md" "$dir" 2>/dev/null | grep -v "过渡期\|历史文档\|L1-L5 即 Artifact Tiers\|<!--" | wc -l)
            
            if [ "$matches" -gt 0 ]; then
                echo -e "    ${YELLOW}⚠️  在 $dir 中发现 $matches 处 '$pattern'（应使用 Artifact Tiers）${NC}"
                WARNING_COUNT=$((WARNING_COUNT + matches))
                
                # 显示前2个匹配项
                grep -r "$pattern" --include="*.md" "$dir" 2>/dev/null | grep -v "过渡期\|历史文档\|L1-L5 即 Artifact Tiers\|<!--" | head -2 | while read -r line; do
                    echo -e "      - $line"
                done
            fi
        done
...
# 6. 检查 Wish 文档中的层级进度表格式
echo -e "\n${BLUE}[6/6] 检查 Wish 文档中的层级进度表格式...${NC}"

WISH_FILES=($(find wishes/active -name "*.md" 2>/dev/null))

for wish_file in "${WISH_FILES[@]}"; do
    if [ -f "$wish_file" ]; then
        # 检查是否包含层级进度表
        if grep -q "层级进度" "$wish_file"; then
            # 检查表格标题是否使用 Artifact Tier
            if grep -q "| 层级 |" "$wish_file"; then
                echo -e "    ${YELLOW}⚠️  $wish_file 使用旧表格标题'层级'（应为'Artifact Tier'）${NC}"
                WARNING_COUNT=$((WARNING_COUNT + 1))
            fi
            
            # 检查是否使用正确的层级名称
            if grep -q "| L1 |\|L2 |\|L3 |\|L4 |\|L5 |" "$wish_file"; then
                echo -e "    ${YELLOW}⚠️  $wish_file 使用数字层级 L1-L5（应使用语义层级名）${NC}"
                WARNING_COUNT=$((WARNING_COUNT + 1))
            fi
        fi
    fi
