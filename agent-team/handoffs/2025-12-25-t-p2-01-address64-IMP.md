...
| 规范要素 | 实现位置 | 备注 |
|---------|----------|------|
| `[F-ADDRESS64-DEFINITION]` | `Atelia.Rbf.<deleted-place-holder>` | 已存在，复用 |
| `[F-ADDRESS64-ALIGNMENT]` | `<deleted-place-holder>.IsValid` + `TryFromOffset` | Rbf 层 `IsValid`，StateJournal 层 `TryFromOffset` |
| `[F-ADDRESS64-NULL]` | `<deleted-place-holder>.Null` / `IsNull` | 已存在，复用 |
