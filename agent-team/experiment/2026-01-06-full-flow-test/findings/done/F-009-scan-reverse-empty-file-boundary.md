# F-009: 空文件与仅有 Genesis Fence 文件的处理边界
...
但 `[R-REVERSE-SCAN-ALGORITHM]` 的伪代码只处理了 `fileLength < GenesisLen` 的情况：
```
1) 若 fileLength < GenesisLen: 返回空
```

**遗漏的边界情况**：
1. `fileLength == 4`（仅 Genesis Fence）：按算法，`fencePos = alignDown4(4 - 4) = 0`，然后在步骤 3a 停止，返回空。这是正确的。

2. `fileLength == 0`（空文件）：这是否是合法的 RBF 文件？按 `[F-GENESIS]` 规定"新建的 RBF 文件 MUST 仅含 Genesis Fence"，空文件不合法。但 `[S-RBF-SCANREVERSE-EMPTY-IS-OK]` 说"空文件"返回 0 元素不抛异常。

3. `fileLength` 在 `(0, 4)` 区间：既不是空文件也没有完整 Genesis，是损坏状态。
