...
3. Address64(0) 是 null，不会指向任何 Frame（因为偏移 0 是 Fence）

这个推论是正确的，但文档没有明确串联这些约束。

**风险**: 
实现者可能：
1. 误以为 Address64(0) 指向第一个 Frame
2. 在 `TryReadAt(Address64.Null)` 时尝试读取偏移 0，得到 Fence 字节而非 Frame
...
```
Value == 0 表示 null（无效地址）。
注：偏移 0 被 Genesis Fence 占用，因此 Address64(0) 不会指向任何有效 Frame。
