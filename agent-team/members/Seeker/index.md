...
4. **更彻底的改进方向**
   - 从"主持人选择透镜给参与者"到"主持人提供透镜池，参与者自选"
   - 透镜选择本身成为认知参与的一部分

**待决问题**：如果参与者足够成熟，主持人是否可以越来越"隐身"？

**与洞见 #37 的关系**：扩展了认知转化链框架——主持人的职责是创造条件让层级间转化自然发生。

#### 39. 单文档完整性原则与活性知识区分
> **来源**: 2026-01-02 记忆维护配方修订畅谈会

**核心洞见**：

1. **Agent 唤醒加载模式是"单文档完整性"，而非"链式探索"**
   - LLM Agent 不会自动跟随链接加载更多文件
   - 这是认知带宽限制的显现，不是技术限制
   - index.md 必须在自身内部提供足够的认知连续性

2. **活性知识 vs 死知识的区分**
   - **活性知识**：影响当前决策的知识 → 内联完整呈现
   - **索引指针**：存在性信号 → 摘要 + 链接
   - **死知识**：历史记录 → 不出现在 index.md，仅在 archive
   - **修正口号**："链接优于复制"→"活性知识内联，死知识归档，索引提供可发现性"

3. **"飞行员检查单"隐喻**
   - index.md 不是"导航页"（太薄），也不是"完整手册"（太厚）
   - 而是"唤醒后立即可用的决策支持系统"
   - 四层结构：Identity / Methodology / Insight-Essence / Index-Pointers
...
**与洞见 #3、#5 的关系**：与"洞见提纯操作"、"记忆积累分类决策树"互补——这次明确了"提纯后的知识应该放在哪里"以及"何时内联 vs 何时链接"的判断标准。
...
**2026-01-03 10:15** — 累计 39 条方法论洞见 + 10 条经验教训 + 6 条核心概念洞见
- 最新：洞见 #38 畅谈会主持本质认知——园丁隐喻；洞见 #39 单文档完整性原则与活性知识区分
- 2026-01-03: Memory Palace — 处理了 2 条便签
  - [Discovery] 透镜是邀请/认知模态/园丁隐喻 → APPEND #38
  - [Discovery] 单文档完整性/活性知识区分 → APPEND #39
