...
> **来源**: 2026-01-13 W-0010 Stage 管理畅谈会

**核心洞见**：讨论 Stage 管理时，易陷入"系统为 Stage 服务"的框架。但本质问题是：**Agent 认知带宽有限条件下的信息分层投影**。Stage 是**手段**不是**目的**。

**栈帧类比**：
| Stage 组件 | 程序栈帧对应 | 信息角色 |
|:-----------|:-------------|:---------|
| `prologue.auto.md` | 保存的寄存器 | 前面的返回值 |
| `shape.md` | 当前栈帧 | 完整局部上下文 |
| `epilogue.md` | 下一次调用的 spec | 向下传递的约束 |

**待验证假设**：摘要应是惰性按需生成（根据当前任务粒度需求），而非提前预制。
