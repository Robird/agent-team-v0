...
**L3→L4 过渡评估框架**（2025-12-30 补充）：
- **L3 条款架构评估三维度**：
  1. 条款间一致性检查（A 的输出是否与 B 的输入对齐）
  2. 可实现性评估（每个 MUST 条款是否有清晰技术路径）
  3. 扩展预留识别（硬编码部分是否有接缝可供未来替换）
- **L4 决策组织模式**：按"关注点"分组（项目结构、库选择、算法选择、错误策略），每个决策点提供"选项表 + 推荐 + 理由"三元组
...
| 层级 | 核心问题 | 判据类型 | 隐喻 |
|:-----|:---------|:---------|:-----|
| L1 Why | 值得做吗？ | 人类价值判断 | 罗盘 |
| L2 Shape | 边界在哪？ | 易用性+完备性 | 门面 |
| L3 Rules | 什么合法？ | 形式化检验 | 法典 |
| L4 Route | 走哪条路？ | 量化评估 | 地图 |
| L5 Craft | 怎么造？ | 可执行性（测试） | 施工 |

**术语提案 C**：Why → Shape → Rules → Route → Craft（待监护人确认）

**"小羊吃草"的病因**：跨层级跳跃无意识——在 L3 讨论中发现 L1 概念问题但继续讨论 L3 → 永远无法收敛。
...
**乔布斯类比映射**：L1 未定时争论 L3 细节 = 战略未定时争论战术。

**哲学审视：五层级的本质是"决策维度的正交分解"**

软件开发决策空间是高维纠缠的，五层级将其分解为：意图(L1) × 界面(L2) × 契约(L3) × 策略(L4) × 实现(L5)。正交意味着"在一个维度上移动，不必同时在其他维度上移动"。

**完备性分析**：
- 安全性、性能等是**横切关注点**，贯穿所有层级，不需要独立层级
- 可能遗漏的 L0（问题验证层）："这是真问题吗？"—— 自研工具可嵌入 L1，面向用户产品可能需要独立

**边界模糊地带的哲学**：模糊地带不是设计缺陷，是连续光谱离散化的必然产物。分类系统的价值不在于消除模糊，而在于让 80% 的情况变得清晰。

**实践建议**：
- 畅谈会开场声明聚焦层级
- 建立跨层问题标记机制：`[L1-BLOCK]` 表示需先解决的概念层问题
- 文档按层级组织：concept.md / api.md / spec.md / implementation.md

**命名优化建议**：Why → What → Rules → How → Build

**实践验证：L2/L3 时态投影模式**（2025-12-31 DocGraph spec.md 创建）：
- **shapeDependency 字段**：显式绑定 L2 版本（如 `api.md@0.1.0`），当 L2 升级时触发 L3 兼容性审查
- **术语窄化（Narrowing）**：L3 可以收窄 L2 术语的语义范围，但不能扩展
- **演进自由度**：L2 和 L3 之间的"空间"就是版本演进的自由度
- **核心发现**："L2 Shape 是天花板，L3 Rules 是地板"——两份文档各司其职，通过 shapeDependency 保持可追溯的演进路径
...
**系统类比**：邮政编码 vs 地名、IP vs 域名、Git SHA vs 分支名、L1 vs Why

**应用场景判断**：
- 叙事讨论 → 语义术语优先
- 表格索引 → 坐标 + 语义组合（如 `L1 Why`）
...
**2025-12-31 19:30** — 累计 34 条方法论洞见 + 10 条经验教训 + 6 条核心概念洞见
- 最新：洞见 #34 开放 Schema vs 封闭 Schema 设计决策框架
- 2025-12-31: Memory Palace — 处理了 3 条便签
  - [Discovery] Normative Stack 时态投影模式 → MERGE #32
  - [Discovery] 最小可实施规范（MVS）框架 → MERGE #15
  - [Discovery] 开放/封闭 Schema 决策框架 → APPEND #34
