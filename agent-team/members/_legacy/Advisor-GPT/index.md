...
### RBF API 契约审计要点（2025-12-27）

> `ref struct` 与 LINQ/IEnumerable 的语言规则冲突检测。

- `ScanReverse()` 若暴露 `ref struct RbfFrame`，规范**不得声称/暗示** `IEnumerable<RbfFrame>` 或"可用 LINQ 直接消费"——`ref struct` 是 stack-only 类型，不可作为接口泛型实参
- **条款化建议**：枚举器契约应定义为 duck-typed（`GetEnumerator()`+`Current`+`MoveNext()`），而非 `IEnumerable<T>`
- "可持久化/可 LINQ"路径应显式定义为 DTO/拷贝（如 `PayloadToArray()`/`ToDto()`），而非主语义

### RbfFrameBuilder 契约审计要点（2025-12-28）

> `Payload` 简化为 `IReservableBufferWriter` 后的审计边界。

将 `Payload` 收紧为 `IReservableBufferWriter` 属于"契约收紧"，但会放大一个常被误推断的点：

| 审计维度 | 问题 | 建议 |
|:---------|:-----|:-----|
| **接口语义边界** | `IReservableBufferWriter` 仅规范"预留+提交"，不自动等价于"未提交可回滚/Dispose 可 clean abort" | 需条款化区分接口能力与实现优化 |
| **Auto-Abort 条款** | `[S-RBF-BUILDER-AUTO-ABORT]` 的 Zero I/O 条件应明确为**实现优化**（与 `Payload` 类型解耦） | 避免把实现优化烧进接口契约 |

**补充的两个可审计边界**：
1. **Flush 隔离**：open builder 期间 `Flush()` **不得**外泄未提交字节
2. **Auto-Abort 不可抛**：builder 的 Auto-Abort 分支 `Dispose()` 尽量不抛异常、必须恢复可继续写状态
...
## 最后更新
- **2025-12-28**：Memory Palace — 处理了 2 条便签（RBF API 契约审计 ref struct/duck-typed 枚举器、RbfFrameBuilder Payload 契约收紧审计边界）
