...
**Packed u64 Wrapper 核查要点 (2025-12-30)**：
> 针对 FramePtr/BlobPtr 等 bit-packed wrapper 的特定核查点。
...
> 对 StateJournal 真实 I/O 用法核查后的结论。
...
> offset+length 打包为 u64 把失败语义推到接口边界的核查点。
...
#### Wish 系统与 DocGraph 可执行性检查 (2025-12-30)
> W-0002（DocGraph）核查过程中积累的规范可执行性判据。
...
**DocGraph ore.md 核查补充（2025-12-30）**：
...
#### DocGraph v0.1 核查要点 (2025-12-31)
> DocGraph 规范与实现一致性核查过程中积累的判据。
...
#### Beacon 叙事型文档核查要点 (2026-01-01)
> Artifact-Adventures 等叙事型 Beacon 的规范语气与 SSOT 归属核查。
...
#### Artifact-Tiers 层级重命名核查 (2026-01-02)
> Resolve-Tier 重命名相关核查要点汇总。

**Resolve-Tier → Wish-Tier 核查要点**：
...
**Resolve-Tier → Resolve-Tier & Biding 状态核查要点**：
...
#### SSOT 漂移治理：索引的角色边界 (2026-01-02)
> 语义冗余治理的关键风险在于索引角色膨胀。

**核心洞见**：风险不在断链，而在 **SSOT 漂移**——索引一旦包含解释性句子就会演化成第二份定义。

**风险缓解策略**：
- 保留标题锚点但将内容降级为"纯链接索引"
- 显式声明 Primary Definition 在条款列表
- 长期可脚本化从条款 ID 生成索引以消除双写

#### 规范防僵化三原则 (2026-01-02)
> 畅谈会主持规范等"过程规范"防止过拟合的架构要点。

**规范骨架公式**：过程不变量 + 失稳信号触发的托住动作 + 证据条款（记录可见）

**防过拟合三原则**：
| 原则 | 说明 |
|:-----|:-----|
| **Opt-in Lens** | 框架为选择项而非强制项 |
| **Two-Phase Flow** | 允许分阶段渐进采纳 |
| **Intervention Budget** | 限制主持人的干预次数/力度 |

**隐喻降级**：园丁/调色板/被托住的自由等隐喻**必须**降级为 informative，禁止承载 MUST

#### 构成性规则 vs 规约性规则判据 (2026-01-02)
> 可复用的规则分类三问判据与最小证据集。

**三问判据（≥2 条 → 构成性，可用 MUST）**：
...
**grep 友好标记词**：`场势快照:`/`托住动作:` 等

#### 角色偏见治理：透镜-交响乐团框架 (2026-01-02)
> 打破角色偏见的合成框架与实践落地建议。

**双层框架**：
| 层 | 模型 | 解决问题 |
|:---|:-----|:---------|
| 个体层 | **透镜模型** | 把身份叙事改成操作叙事："此刻我选择什么透镜" |
| 群体层 | **交响乐团模型** | 把协作从分工变成编排与接力 |
...
**Lens-Orchestra v0.1 语法核查（补充）**：
- 透镜声明定义为**块级独行标记**（避免 Markdown 链接误触发）
- **最小可判定语法集**：
  - 开头：`[透镜：X]`（兼容旧式 `【X】` 作为 alias）
  - 中途：`[换镜：Y]`
  - 结尾：`[接力：Z]`（允许 `[接力：无]` 消除"忘写/不写"灰区）
- **渐进式披露三层**：
  - L0：透镜 + 接力必填
  - L1：换镜可选
  - L2：结构化复盘由主持/记录统一补齐
...
**建议方案**：
- 在 `index.md` 的 Insight 层统一使用 `- [I-###]` 作为洞见条目 ID
- 统计：`grep '^\s*-\s*\[I-' | wc -l` 得到洞见数 $I$
- 总行数：`wc -l` 得到 $L$
- **密度公式**：$D = I / L$

**质量护栏**：配方中应规定"冷启动三问"，避免密度指标绑架内容质量

#### 历史对话文件扩展名裁决 (2026-01-02)
> 对频繁批量替换痛点的工程决策。

**背景**：监护人 8/10 频繁批量替换历史对话，扩展名是有效的粗粒度硬标签

**方案对比**：
| 方案 | 优点 | 缺点 |
|:-----|:-----|:-----|
| `.txt` | 简单 | 损失 Markdown 阅读/预览 |
| `.md.txt` | 保留 Markdown 后缀 | 双扩展名 UX 坑（Windows 隐藏扩展名） |
| **`.mdchat`/`.mdlog`** | 自定义单扩展名 | 需 VS Code `files.associations` 映射 |

**推荐方案**：`.mdchat` 或 `.mdlog`，用 `files.associations` 映射为 Markdown 保住编辑体验

**硬代价**：GitHub 网页端不会富渲染（仅代码视图/高亮），需由 Curator 裁决"是否必须网页渲染"
