...
**LLM 调用的三层模型（实现映射）**
- **ICompletionClient**：厂商 API 适配层（OpenAI/Anthropic/Google 等）
...
### 2025-12-21 补充：规范“可测试性阻塞点”三分法（适用于所有基础设施规格）

- **条款冲突（self-contradiction）**：同一对象在同一状态下，既要求“所有访问必须失败”，又要求“某些查询必须不失败”，会导致测试无法判定。修复方式是引入明确的 API 分层（元信息 vs 语义数据访问）并列出白名单。
- **失败通道二义（Result vs Exception）**：若规范不钉死“可预期失败走哪个通道”，实现会分叉，测试也无法统一断言。修复方式是选择一个主通道，并明确另一个仅用于 bug/不变量破坏。
- **返回形态未钉死（null vs NotFound）**：这类“看似小”但会撕裂调用方心智模型；MVP 阶段应优先唯一性而非完美抽象。
