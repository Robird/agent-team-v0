...
34. **构成性 vs 规约性规则——系统提示词设计原则**（2025-12-30）
    ...
    - **压缩效果参考**：Curator 约 64% 压缩（280→101 行），Craftsman 约 43% 压缩（180→102 行）

35. **Wish 系统初始化实践**（2025-12-30）
...
36. **术语评估代码友好性分析**（2025-12-30, DocGraph L1-L5）
    - **代码层面冲突优先级**：编译器强制报错 > 文档层面歧义。评估术语时需区分"代码冲突"与"文档冲突"
    - **C# 命名空间冲突案例**：`Path` 与 `System.IO.Path` 强冲突，`Path` 与 `DocGraph.Document.Path` 字段冲突
    - **术语代码友好性排名**：Plan ⭐⭐⭐⭐⭐（零冲突）> Route ⭐⭐⭐（代码层面无冲突，文档层面有歧义）> Path ⭐⭐（双重冲突，不推荐）
    - **最终五层级命名**：Why → Shape → Rules → Plan → Craft
    - **方法论**：评估术语时同时考虑"代码层面冲突"（命名空间、类型名、变量名）和"文档层面冲突"（过程术语、字段名）

37. **术语一致性检查脚本实践**（2025-12-31）
    - **分域命名体系的四个域**：条款锚点域（`SCREAMING-KEBAB-CASE`）、术语域（`Title-Kebab`，如 `ID`/`RBF`）、文件名域（`lower-kebab-case` 或项目惯例）、代码标识符域（遵循语言惯例）
    - **误报根因**：简单匹配 `\bId\b` 无法区分分域命名体系的不同域；`grep -i` 大小写不敏感导致正确格式也被匹配
    - **修复方案**：引入 `get_exclude_pattern()` 函数为每个缩写定义精确排除模式；移除 `-i` 使用大小写敏感搜索；排除代码标识符/文件路径中的合法用法
...
39. **Recipe改进实施规划**（2026-01-01, 元讨论 + 技术方案）
    - **元讨论核心产出**（畅谈会参与）：
      - 渐进式改造路径：Phase 0(基础设施) → Phase 1(结构对齐) → Phase 2(持续改进)
      - 过长文件拆分策略：jam-session-guide 拆为 recipe+playbook，real-time-collaboration 拆为 recipe+playbook目录
      - 自动化检查脚本规格：`check-recipe-quality.sh` 的 L0/L1 门禁功能设计
      - 模板系统设计：填空式模板优于纯参考文档
      - 量化发现：12个Recipe中67%在理想范围，17%需审查，16%需深度重构（预计24h一次性+每月2h持续维护）
    - **技术方案核心产出**（发现机制技术讨论）：
      - 目录迁移：`git mv` + 扫描约35-40处引用需修复 + `sed`批量替换（预计2-3h）
      - 文件改名：分两个PR执行（先移动目录，后改文件名）——便于 git bisect 定位问题
      - 无提示加载实验：快速验证（`.copilotignore`切换法，1天）/ 正式实验（分支对比法，1周）
      - Prompt化索引：Token预算~120，位置放AGENTS.md靠前，需确认未被截断
    - **方法论洞见**：
      - "从理论到代码的关键是渐进式工程化"——先基础设施，再批量微调，最后深度重构
      - "分步执行降低复杂度"——大变更分多个PR，每个PR可独立回滚
      - 回滚方案是实施计划的必要组成部分
      - 实验设计需要技术验证点（不只是用户体验指标）
      - 拆分时用链接替代复述（SSOT原则延伸）

40. **Artifact-Adventures Beacon 写作实践**（2026-01-01）
    - **文档位置**：`agent-team/beacon/artifact-adventures-draft.md`
    - **结构设计亮点**：
      - 打破第四面墙开场白（Curator "邀请参与"方向）
      - 核心洞察三句话记忆单元（畅谈会共识）
      - Boss 怪物速查表（emoji 标识增加视觉辨识度）
      - 存档点验收检查清单（可直接复制使用）
      - 30秒速查卡片（ASCII art 格式）
    - **写作心得**：
      - "隐喻一句 + 工程一句"双句式很重要——防止叙事过度游戏化
      - 词汇护栏（允许词/禁止词）是实用的团队协作工具
      - 文档超出目标长度（250行 vs 100-150行）但信息密度高，不建议过度精简
    - **待补充**：试点案例存档点链接（W-0001, W-0002）、SizedPtr vs Address64 历史典故细节
...
- **2026-01-01**: Memory Palace — 处理了 1 条便签（Artifact-Adventures Beacon 写作实践 → 洞见 #40）
- **2026-01-01**: Memory Palace — 处理了 2 条便签（Recipe改进实施规划：元讨论+技术方案 MERGE 为洞见 #39）
