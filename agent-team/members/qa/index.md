...
- **场景**：两个规范文档描述同一个值（如 SizedPtr.Length 和 HeadLen），但缺少明确的等价声明
- **洞见**：
...
#### [I-QA-011] 文档迁移的验证真空与三层守卫策略（2026-01-07）

> 来源：SizedPtr 迁移 3 周重复失败回顾

- **场景**：SizedPtr 迁移完成后被意外回退，3 周重复失败
- **问题诊断**：
  1. **代码有单元测试作为回归守卫，但文档级迁移缺乏等价机制**
  2. Wish 系统记录意图（Plan-Tier），但不阻止回退（非 Rule-Tier）
  3. 提交 diff 看起来像"改进"，reviewer 难以识别是"倒退"
- **三层迁移守卫策略**：
  | 层级 | 机制 | 作用 |
  |:-----|:-----|:-----|
  | **Grep Guard** | 脚本检查禁止出现的旧术语 | 自动拦截回退 |
  | **文档注释** | 机器可读的迁移锁定标记 | 声明性约束 |
  | **Baseline Tracking** | 记录预期的术语出现次数 | 检测意外变化 |
- **验证技巧**：为重要迁移建立三层守卫，而非依赖人工记忆

#### [I-QA-012] DSL 迁移的三层验证策略（2026-01-09）

> 来源：AI-Design-DSL 格式迁移文档验证

- **场景**：验证 AI-Design-DSL 格式迁移的文档
- **三层验证策略**：
  | 层级 | 验证内容 | 方法 |
  |:-----|:---------|:-----|
  | **语法层** | `### decision\|design\|hint`、Clause-ID（全大写）、Term-ID（Title-Kebab） | grep 批量检查 |
  | **引用层** | `@[CLAUSE-ID]` 引用 vs 定义列表 | 提取对比，找悬空引用 |
  | **语义层** | 同一语义在不同层级文档中的表述一致性 | 跨文件条款对比 |
- **验证技巧**：DSL 迁移验证可复用此三层检查清单

#### [I-QA-013] 文档一致性修复的"纯引用模式"验证清单（2026-01-09）

> 来源："语义重复 → 纯引用"修复验证

- **场景**：验证"语义重复 → 纯引用"修复是否正确
- **五点验证清单**：
  1. **锚点保留**：条款 ID + 标题必须保留（供外部引用）
  2. **SSOT 可达**：引用目标必须存在（grep 验证）
  3. **不重复书写**：搜索原有关键词（如 MUST 规范、实现路径）确认已移除
  4. **概念简述**：允许保留一句话概念（如"ticket"），帮助读者理解
  5. **单向依赖**：被引用文件不应反向引用
- **验证命令模式**：
  ```bash
  grep -E "原关键词模式" target.md  # 确认重复内容已移除
  grep "CLAUSE-ID" source-of-truth.md  # 确认引用目标存在
  grep "target.md" source-of-truth.md  # 期望 0 matches（单向依赖）
  ```

#### [I-QA-014] 转义字符污染模式（2026-01-09）

> 来源：Markdown 代码块编辑异常

- **场景**：Markdown 代码块内的反引号在某些编辑流程中被自动转义
- **症状**：`\`` 而非 `` ` ``
- **检测方法**：`grep -E '\\`' file.md`
...
#### [I-QA-017] 批量条款ID改名的验证策略（2026-01-12）

> 来源：条款ID批量改名验证实践

- **场景**：验证批量条款ID改名（如 19 个条款）的完整性和正确性
- **三层检查法**：
  | 层级 | 检查内容 | 方法 |
  |:-----|:---------|:-----|
  | **新ID存在性** | 确认定义已添加 | `grep '\[NEW-ID\]' file.md` |
  | **旧ID残留检查** | 确认无残留 | `grep '\[OLD-ID\]' file.md`（排除归档/审计报告） |
  | **覆盖率统计** | 识别有意保留 vs 遗漏 | 对比审阅清单 |
- **统计口径差异识别**：影响分析预期数与实际替换数差异通常有三个来源：
  1. **SSOT 内部合并**：定义+引用可能被分开统计，改定义后引用自动生效
  2. **引用文件虚高**：grep 匹配不等于需要更新（元文件/历史文件可豁免）
  3. **统计口径**：可能统计"出现次数"而非"需改位置数"
- **前缀碰撞陷阱**：旧ID可能是新ID的前缀（如 `S-FORMAL-PARSING` vs `S-FORMAL-PARSING-SUPPORTED`），应使用 `OLD-ID]` 正则避免误匹配
- **验证技巧**：
