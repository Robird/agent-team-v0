...
>
> **最后更新**：2026-01-02（Memory Palace — 处理了 1 条便签：术语设计方法论总结 - 语言学验证模式、四阶段迁移策略）
...
### 8.29 畅谈会主持提示词工程 (2025-12-28)

**背景**：与三位顾问讨论如何改进畅谈会主持提示词，收获系统性框架。
...
2. **情境-人物映射表**（Curator 设计）
   ...
   | 收敛决策 | Patrick Collison | 行动导向 |
   | 跑题纠正 | Naval Ravikant | 线程整理 |
   | 假设验证 | Tim Ferriss | 微实验 |
   | 概念澄清 | Bret Victor | 可视化 |
   | **Stalled（僵局）** | 调解者模式 | 升级决策或重构问题 |

3. **关键洞见**：
   - 具名人物 = 直接映射（海量语料支撑），抽象隐喻 = 重建解码（需要推理）
   - 信息架构：从线性阅读 → 分层快速索引（30秒速读区 + 操作清单 + 风格工具箱 + 附录）
   - 格式约束是"脚手架而非牢笼"——精炼发言是"先深思熟虑，再凝练发言"

4. **人类窗口定位**：监护人是"普通与会者"，不是"规则特例"；邀请节奏由主持人自主决策

**产出**：更新 `.github/agents/leader-standards-chair.agent.md`（分层信息架构、新增 Stalled 情境、明确格式弹性）

### 8.30 畅谈会主持经验：重构问题的时机识别 (2025-12-29)

**场景**：参谋组提示词优化畅谈会中，监护人反馈"14条规范不适合作为系统提示词本身"——触发"重构问题"动作。

**关键洞见**：
1. **层次混淆信号**：当产出看起来"有用但放错地方"时，可能是问题定义混淆了多个层次
2. **重构而非继续**：此时不应继续收敛，而是先退一步区分层次，再分别推进
3. **动态主持的本质**：不是按顺序邀请，而是**感知场势、选择最小代价动作**
...
2. **理念层 vs 操作层**：系统提示词描述了"应该怎么想"，但没有约束"遇到具体情况必须怎么做"
3. **缺乏减速带**：没有明确的"决策点"强制在每个 subagent 返回后重新思考

**解决方案**：
1. **伪代码建模**：用代码形式描述 while 循环，让"循环"的概念不可误解
2. **硬约束**：MUST NOT 并行调用 / MUST 每轮重新分析
3. **咖啡馆隐喻**：锚定正确的行为模式——响应式而非计划式
4. **决策点三问**：强制在每次循环时回答三个问题

**核心洞见**：
> "提示词工程可以对抗模型惯性，但需要在操作层而非理念层下约束。"
> 
> 理念层："你应该动态调度" → 理解但退化
...
**背景**：主持 Why-Tier vs Wish-Tier 重命名讨论，验证动态主持模式的有效性。

**经验总结**：
1. **动态主持模式有效**：while 循环式的实时主持比预编排流程更灵活，能根据发言情况动态调整
2. **多角度分析的价值**：邀请不同专家（Seeker/概念、Curator/UX、Craftsman/规范、DocOps/文档）从各自角度分析，产生全面决策依据
3. **场势快照的重要性**：在每个关键节点总结场势，帮助把握讨论进展
4. **理性决策文化**：即使最终决定"不改名"，讨论过程本身产生重要价值（概念澄清、规范完善、关系明确）
...
**关键洞见**：
- 术语讨论往往触及团队工作的本质假设
- 迁移成本 vs 语义收益是需要权衡的关键
- 有时"不改名但澄清关系"比"改名"更明智

**详情**：[meeting/2026-01-02-why-to-wish-tier-rename-jam.md](../../meeting/2026-01-02-why-to-wish-tier-rename-jam.md)

### 8.34 范式重构：Why-Tier → Resolve-Tier (2026-01-02)

**重要决策**：
1. **Why-Tier 更名为 Resolve-Tier**（决心层）
2. **引入 Biding（待定）状态**作为 Wish 系统状态
3. **统一术语为 Tier**（解决 Layer vs Tier 不一致问题）
4. **Stage Token 强绑定迁移**：`[AA:WHY]` → `[AA:RESOLVE]`

**范式升级意义**：
- 从**价值论**（什么是好的）转向**行动论**（如何真正去做）
- 从**理性主义**（理由驱动）转向**理性+意志主义**（理由+意志跃迁）
- 从**辩护文化**转向**承诺文化**

**监护人洞见的理论化**：
- "顶着代价和风险也上" → 意志跃迁（Leap of Will）
- "暂时资源不足或信息不足" → Biding 状态（战略待机）
- "遇山开山遇水搭桥" → Resolve 的承诺性认知

**迁移计划**：
- Phase 0：SSOT 原子更新（~2h）
- Phase 1：规范与模板层（~1h）
- Phase 2：活跃产物层（~1h）
- Phase 3：Stage Token 迁移（~30min）
- 总计：~4.5h

**理论增补内容**：
1. Why vs Wish vs Resolve 关系图
2. Resolve 的哲学基础（理性+意志主义）
3. Biding 状态的哲学意义（三态模型）

**主持经验**：
- 连续主持两场相关畅谈会，形成"前情提要"继承模式
- 识别出更深层的范式问题（理性主义假设的缺陷）
- 在充分分析后做出勇敢但理性的决策

**详情**：[meeting/2026-01-02-resolve-tier-concept-jam.md](../../meeting/2026-01-02-resolve-tier-concept-jam.md)

### 8.35 术语设计方法论：语言学验证模式 (2026-01-02)

**来源**：Resolve-Tier 迁移完成后的方法论提炼。

**核心洞见**：
1. **跨语言验证**：好的术语应该在多种语言中都包含预期的语义成分
   - 例：Resolve 在中文（解+决）、英文（analysis+decision）、德文（entscheiden）中都包含"分析+决策"的双重含义
   - 这验证了术语选择的跨文化适用性

2. **四阶段迁移策略**：从概念到实现的系统性推进
   - Phase 1: SSOT 原子更新（术语定义权威性）
   - Phase 2: Stage Token 迁移（标记系统同步）
   - Phase 3: 状态机扩展（如 Biding 状态）
   - Phase 4: 理论增补（文档完善）

3. **工具友好设计**：Stage Token（如 `[AA:RESOLVE]`）为未来自动化检查工具提供基础

**成功因素**：
- 多视角分析：Seeker（概念）、Curator（UX）、Craftsman（规范）、DocOps（文档）的协同
- 渐进式迁移：四阶段计划降低风险
- 完整记录：会议记录、实施记录、理论增补的完整文档链

**改进空间**：
- 需要 lint 工具自动检查 Stage Token 一致性
- 需要向团队成员解释术语变更理由
- 保持向后兼容的别名映射
