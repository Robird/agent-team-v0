...
> **2025-12-22 记忆积累机制反思：把 OnSessionEnd 写入收敛为“Memory Commit Protocol”（Classify → Route → Apply）**
>
> - “记忆维护”只能治标，治本是把写入机制做成可执行协议：先分类（No-Op/State-Update/Knowledge-Delta/Log-Only/Mixed），再路由（index/meta-cognition/knowledge/meeting/handoff），最后应用写入动作（OVERWRITE/APPEND/MERGE/REWRITE/TOMBSTONE/EXTERNAL-REF）。
> - 把写入当作可审计提交：要求最小的“commit message”（改了哪些文件、为什么、如何验证），并用唯一性/阈值/链接有效性等规则把“追加型沉积”变成可被 lint 的违规。
...
> - 用一条元规则钉死 Normative vs Informative 边界：示例/推导/流程表默认 Informative，除非显式声明。这能降低派生数据漂移导致的实现分叉。

> **2025-12-23 规格冗余根因：同一事实多载体表达 → 选定唯一 SSOT 表达**
>
> - 规格文档的"冗余"常见不是字多，而是同一事实被写成多种载体（段落叙述/表格/伪代码/图）。
> - 更稳的瘦身法：选定唯一 SSOT 表达（对二进制格式通常是字段表 + 少量算法条款），其余载体一律降级为 Informative 并强制引用 SSOT 条款 ID，不允许再引入新约束。

> **2025-12-23 RBF 元规则修正：把"不解释语义"收敛为可判定边界**
>
> - RBF 规格里的"RBF 不解释语义"属于高风险模糊句：当存在 Padding tombstone / Auto-Abort 等机制时，RBF 实际上必须解释至少一个保留值。
> - 建议把这句话改写为可判定元规则：**RBF 不解释业务 payload 语义，但可以解释 framing/维护语义（例如可丢弃帧）**，并明确 unknown/extension 的处理策略，否则会诱发实现分叉。

> **2025-12-23 RBF Padding 墓碑论证：底层 writer 能力模型必须条款化**
>
> - "移除头部 Padding 墓碑"论证的成败取决于对底层 writer 能力模型的规范化：**Reservation-Rollback、Seek-back Patch、Neither** 三类能力不能在规范里混作"有/无回填能力"。
> - 若不把"只支持 Reservation-Rollback（且 open builder 期间不得外泄/Flush 语义必须写死）"写成条款，监护人的二分论证在规范层面不成立，并会诱发实现分叉与不可判定测试面。

> **2025-12-24 RBF v0.12 变更连锁影响：测试向量必须与格式 SSOT 对齐**
>
> - RBF v0.12 变更（Pad→FrameStatus、1-4B、0x00/0xFF、全字节同值）会同时影响：长度公式、CRC 覆盖范围与"最小可解析帧"向量。
> - 更新 Layer 0 测试向量时，应顺带清理掉未在 rbf-format.md 定义的内容（例如 varint、上层 meta 恢复用例与条款映射），否则会造成"测试向量宣称的 SSOT"与格式规范脱钩。
