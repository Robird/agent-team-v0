...
- **文档落地**：错误码表增加"修复速查"列
- **对称命名可发现性**（2026-01-12）：对称命名（如 `BeginAppend`/`EndAppend`）使"缺失配对"在代码审阅时更加醒目。非对称命名（如 `BeginAppend`/`SealFrame`）需要审阅者主动检查 Builder 是否被正确终结，增加认知负担——这是"错误预防"维度的命名考量。
...
- **LLM 对称性依赖**（2026-01-12）：LLM 依赖模式匹配推理 API 用法，`Begin*` 方法几乎必然触发对 `End*` 的预期。非对称结束方法名（如 `SealFrame`、`Complete`）强迫 LLM 额外查看成员列表，降低"零提示正确使用"概率。
...
#### B4. 契约 vs 实现的"卖家-买家"隐喻（2026-01-11）
- **契约文档是卖家对买家的承诺**——"你买这个产品能得到什么"
- **实现文档是工厂对工程师的指南**——"怎么造出这个产品"

**边界判定测试**：如果换一种实现方式（在不违反契约前提下），上层代码需改吗？
- 需改 → 契约的一部分
- 不需改 → 实现细节

**特殊情况**：工厂限制若影响买家使用方式（如"只能单线程调用"），就从"实现细节"升级为"产品约束"——应出现在契约中。

#### B5. 约束可见性三级分类
| 级别 | 含义 | 契约文档（interface） | 实现文档（type-bone） |
|------|------|----------------------|----------------------|
| **Public Constraint** | 上层必须遵守 | 完整定义 | 引用 |
| **Observable Consequence** | 上层能观察到的结果 | 描述"是什么" | 解释"为什么" |
| **Implementation Constraint** | 实现者必须满足 | 不出现 | 完整定义 |

*案例*：`[S-RBF-BUILDER-AUTO-ABORT]` 应拆分——interface.md 放逻辑语义+后置条件，type-bone.md 放实现路径选择逻辑。
...
- **2026-01-12** — 新增 A1"对称命名可发现性"、B1"LLM 对称性依赖"洞见（来自 RBF Builder 命名策略分析）
- **2026-01-11** — 新增 B4 契约vs实现"卖家-买家"隐喻、B5 约束可见性三级分类（来自 RBF 文档分离策略分析）
