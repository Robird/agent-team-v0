...
- **[F-WISH-FRONTMATTER-REQUIRED-FIELDS]** MUST：frontmatter 必须包含以下必填字段：
  ...
  - `title`: 标题，长度 ≤ 80 字符
  - `status`: 状态枚举，值为 `Active` | `Biding` | `Completed` | `Abandoned`
  - `owner`: 责任人/角色
...
- **[F-WISH-DIR-ROOT]** MUST：Wish 实例目录根 MUST 为仓库根下的 `wish/`。
- **[F-WISH-DIR-INSTANCE]** MUST：每个 Wish MUST 对应一个实例目录：`wish/W-XXXX-<kebab-case-slug>/`。
- **[F-WISH-DIR-ENTRYFILE]** MUST：每个 Wish 实例目录 MUST 包含入口文件 `wish.md`。
- **[F-WISH-DIR-PANELS-SEPARATION]** MUST：派生视图/汇总面板 MUST 放在仓库根下 `wish-panels/`，不得混入 `wish/`。
- **[S-WISH-STATUS-ENUM]** MUST：`status` 枚举为 `Active | Biding | Completed | Abandoned`。
- **[S-WISH-STATUS-SSOT]** MUST：Wish 的 `status` 语义以 `wish.md` frontmatter 字段为准，不再从目录推导。

### §3.2 文件命名

- **[F-WISH-INSTANCE-DIR-NAMING]** MUST：Wish 实例目录名 MUST 使用稳定格式：`W-XXXX-<kebab-case-slug>`。
  - `W-XXXX`: 4 位数字序号（如 `W-0001`）
  - `<kebab-case-slug>`: 简短描述性 slug
- **[F-WISH-INSTANCE-DIR-ID-MATCH]** MUST：实例目录名中的 `W-XXXX` MUST 与 `wish.md` frontmatter 的 `wishId` 一致。
- **[S-WISH-INSTANCE-SLUG-STABILITY]** SHOULD：slug 应保持稳定，避免因标题变化而重命名。

---

## §4 index.md 条款

- **[S-WISH-PANELS-DERIVED-VIEW]** MUST：`wish-panels/` 下的汇总面板定义为**派生视图**，可从 `wish/**/wish.md` 扫描重建。
- **[S-WISH-PANELS-SHORTEST-PATHS]** SHOULD：面板 SHOULD 提供以下两条最短路径：
  - "我该从哪开始读"（Top-N active wishes）
  - "当前阻塞是什么"（Blocked issues）
- **[S-WISH-PANELS-GENERATED]** SHOULD：面板应由工具生成（例如 DocGraph Visitor），避免手工维护漂移。
